...
/SEARCH claude.md
/SEARCH *.ts
/SEARCH ext:json memory

# Search with wildcards and filters
/SEARCH "src/*.test.ts"
/SEARCH "ext:exe;dll size:>1mb"
/SEARCH "parent:C:\mcp-servers *.md"
...
1. Parse the query and flags from `$ARGUMENTS`
2. If `--fzf` flag is present, use fzf tools
3. If `--content` flag is present, use `fuzzy_search_content`
4. If `--info` flag is present, use `get_file_info`
5. Otherwise, use Everything `search` tool
