...
- [ ] **Automate `bun.lock` on Dependabot PRs — this is now costing real time every cycle.**
      ...
      **Third data point, 2026-09-15 (memoryjs 4.2.0 uptake):** the manifest already declared
      `^4.0.0`, so 4.2.0 was in range — but `bun.lock` still pinned 4.0.0 and that is what
      installed. A satisfied semver range is not an uptake; the lock decides. The bump needed a
      hand-run `bun install` here too, which is the same missing automation seen from the other
      side: the lock does not move on its own whether Dependabot or a human moves the manifest.
